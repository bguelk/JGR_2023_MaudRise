...
if __name__ == '__main__':
    ...
    for year in range(years,yeare):   
        print(year)
        for mon in [str(i).zfill(2) for i in range(1,13)]:
            ds1 = xnemogcm.open_nemo_and_domain_cfg(nemo_files=list(path_in_mean.glob('*mean_fullsim_grid*.nc')), domcfg_files=['./1_domain_cfg_50levels.nc'],nemo_kwargs=dict(datadir='./'))
            ...
            print('compute Tmax')
            # computing the maximum Temperature below the MLD for the modelled regio
            # the output fields are called " halo"
            ...
            # compute spatial mean values of the Tmax in the Halo and Taylor Cap
            ...
            # compute the spatial mean of T properties in the TC and Halo
            T_mean_ha=ds.thetao.where(ds.ha_mod==1).mean(dim={'x_c','y_c'})
            T_mean_tc=ds.thetao.where(ds.mask_tc==1).mean(dim={'x_c','y_c'})

            T_mean_ha=T_mean_ha.rename("T_mean_ha")
            T_mean_ha=T_mean_ha.assign_attrs(long_name='spatial mean of T in the modified Halo region')
            T_mean_ha.to_netcdf(Path(path_out)/f'T_mean_ha_{year}{mon}_m36.nc')
            T_mean_tc=T_mean_tc.rename("T_mean_tc")
            T_mean_tc=T_mean_tc.assign_attrs(long_name='spatial mean of T in the Taylor Cap region')
            T_mean_tc.to_netcdf(Path(path_out)/f'T_mean_tc_{year}{mon}_m36.nc')

            # compute the spatial mean of T properties in the TC and Halo
            S_mean_ha=ds.so.where(ds.ha_mod==1).mean(dim={'x_c','y_c'})
            S_mean_tc=ds.so.where(ds.mask_tc==1).mean(dim={'x_c','y_c'})
            S_mean_ha=S_mean_ha.rename("S_mean_ha")
            S_mean_ha=S_mean_ha.assign_attrs(long_name='spatial mean of S in the modified Halo region')
            S_mean_ha.to_netcdf(Path(path_out)/f'S_mean_ha_{year}{mon}_m36.nc')
            S_mean_tc=S_mean_tc.rename("S_mean_tc")
            S_mean_tc=S_mean_tc.assign_attrs(long_name='spatial mean of S in the Taylor Cap region')
            S_mean_tc.to_netcdf(Path(path_out)/f'S_mean_tc_{year}{mon}_m36.nc')

            # compute the spatial mean of MLD properties in the TC and Halo
            MLD_mean_ha=ds.mldr10_1.where(ds.ha_mod==1).mean(dim={'x_c','y_c'})
            MLD_mean_tc=ds.mldr10_1.where(ds.mask_tc==1).mean(dim={'x_c','y_c'})
            MLD_mean_ha=MLD_mean_ha.rename("MLD_mean_ha")
            MLD_mean_ha=MLD_mean_ha.assign_attrs(long_name='spatial mean of MLD in the modified Halo region')
            MLD_mean_ha.to_netcdf(Path(path_out)/f'MLD_mean_ha_{year}{mon}_m36.nc')
            MLD_mean_tc=MLD_mean_tc.rename("MLD_mean_tc")
            MLD_mean_tc=MLD_mean_tc.assign_attrs(long_name='spatial mean of MLD in the Taylor Cap region')
            MLD_mean_tc.to_netcdf(Path(path_out)/f'MLD_mean_tc_{year}{mon}_m36.nc')

            # extract N2 spatial mean for Taylor Cap and Halo
            N2_mean_ha=ds.bn2.where(ds.ha_mod==1).mean(dim={'x_c','y_c'})
            N2_mean_tc=ds.bn2.where(ds.mask_tc==1).mean(dim={'x_c','y_c'})
            N2_mean_ha=N2_mean_ha.rename("N2_mean_ha")
            N2_mean_ha=N2_mean_ha.assign_attrs(long_name='mean N2 over the modified Halo region')
            N2_mean_ha.to_netcdf(Path(path_out)/f'N2_mean_ha_{year}_m36.nc')
            N2_mean_tc=N2_mean_tc.rename("N2_mean_tc")
            N2_mean_tc=N2_mean_tc.assign_attrs(long_name='mean N2 over the Taylor Cap region')
            N2_mean_tc.to_netcdf(Path(path_out)/f'N2_mean_tc_{year}_m36.nc')



            print('compute heatflux for the whole model domain')
            ...
            # Integrate the temperature for each level
